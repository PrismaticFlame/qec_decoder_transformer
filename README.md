...
DETAILS ON QEC-PIPELINE

The Dockerfile and docker-compose contains an image to run the transformer model using a GPU in a docker container. Build and start the transformer-dev service to run transformer models using CUDA and GPU.

To run the complete pipeline of simulatng Stim data and predicitng error using MWPM and transformer error correction algorithms, run the pipeline.py file in the src folder. 

The code can be changed to change number of rounds, number of shots, etc. The generated data will go through the the MWPM algorithm and will also get trained on the QEC transformer model, and the results (logical error rates) are compared through graphs generated by the code.
