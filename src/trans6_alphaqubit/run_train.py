#!/usr/bin/env python3
"""
run_train.py - Train single-basis AlphaQubit-like QEC Transformer (trans6)

Changes from trans6:
  - d_model = 256 (up from trans5's 128, matches AlphaQubit paper)
  - 4-input StabilizerEmbedding (measurement, event, leakage, event_leakage)
  - Final-round treatment with on-basis/off-basis distinction
  - Self-contained: all modules local (no trans3 dependency)
...
def load_data(
    ...
    # Load pre-computed measurements if available (trans6 data gen stores these)
    train_meas = train_data.get("meas_hard", None)

    val_samples = val_data.get("det_soft", val_data.get("det_hard"))
    val_labels = val_data.get("obs", val_data.get("labels"))
    val_meas = val_data.get("meas_hard", None)
...
def main():
    parser = argparse.ArgumentParser(
        description="Train single-basis AlphaQubit-like QEC Transformer (trans6: ReadoutResNet + next-stab schedule)",
    ...
    parser.add_argument("--d_model", type=int,
                        help="Model hidden dimension (default: 256 for trans6)")
    ...
    # Build model config with trans6 defaults
    model_cfg = ModelConfigScaling()
    # Trans6 default: d_model=256 (matches AlphaQubit paper scaling experiments)
