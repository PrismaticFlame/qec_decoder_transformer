#!/usr/bin/env python3
"""
sweep_physical_error_rates.py - Train trans6 at each physical error rate and compare vs MWPM

For each physical error rate p:
  1. Generate fresh basis-separated data using Stim
  2. Train a trans6 model from scratch
  3. Run MWPM on the same validation data
  4. Record both LERs

Produces a plot comparing trans6 vs MWPM across error rates.

Usage:
    python src/trans6_alphaqubit/sweep_physical_error_rates.py
    python src/trans6_alphaqubit/sweep_physical_error_rates.py --basis z --shots 100000
    python src/trans6_alphaqubit/sweep_physical_error_rates.py --num_steps 50000

Output:
    src/trans6_alphaqubit/checkpoints/sweep_p/sweep_results.png
    src/trans6_alphaqubit/checkpoints/sweep_p/sweep_results.json
"""
...
# All imports are local to trans6_alphaqubit (self-contained)
...
def train_and_eval_trans6(
    dataset_dir: Path, basis: str, distance: int,
    num_steps: int, eval_every: int, batch_size: int,
    obs_rate: float,
) -> float:
    """Train trans6 from scratch on the generated data and return best LER."""
    rounds = distance * 2
    ...
    # Build model config (trans6 defaults)
    ...
    train_dataset = SyndromeDataset(
        ...
        input_mode="hard",
        measurements=train_data.get("meas_hard", None),
    )
    val_dataset = SyndromeDataset(
        ...
        layout_json_path=layout_path,
        input_mode="hard",
        measurements=val_data.get("meas_hard", None),
    )
...
# Plotting
# ---------------------------------------------------------------------------
def plot_results(ps, mwpm_lers, trans6_lers, shots, basis, distance, output_path):
    """Create comparison plot."""
    fig, ax = plt.subplots(figsize=(10, 7))

    ps = np.array(ps)
    mwpm_lers = np.array(mwpm_lers)
    trans6_lers = np.array(trans6_lers)

    # Error bars (binomial standard error)
    std_mwpm = (mwpm_lers * (1 - mwpm_lers) / (shots * 0.2)) ** 0.5  # val is 20% of shots
    std_t5 = (trans6_lers * (1 - trans6_lers) / (shots * 0.2)) ** 0.5

    ax.errorbar(ps, mwpm_lers, yerr=std_mwpm, fmt="o-", color="#2ecc71",
                linewidth=2.5, markersize=7, capsize=4, label="MWPM (pymatching)")

    ax.errorbar(ps, trans6_lers, yerr=std_t5, fmt="s-", color="#2e75b6",
                linewidth=2.5, markersize=7, capsize=4, label="Trans5 (trained per p)")

    # Shade region where trans6 beats MWPM
    below = trans6_lers < mwpm_lers
    if below.any():
        ax.fill_between(ps, mwpm_lers, trans6_lers, where=below,
                        alpha=0.15, color="#2e75b6", label="Trans5 < MWPM")

    # Shade region where MWPM beats trans6
    above = trans6_lers > mwpm_lers
    if above.any():
        ax.fill_between(ps, mwpm_lers, trans6_lers, where=above,
...
def main():
    parser = argparse.ArgumentParser(
        description="Sweep physical error rates: train trans6 at each p, compare vs MWPM",
    )
    ...
    for i, p in enumerate(ps):
        ...
        # 3. Train trans6 and evaluate
        print(f"\n  Training trans6 ({args.num_steps} steps)...")
        t5_ler = train_and_eval_trans6(
        ...
        result = {
            ...
            "mwpm_ler": mwpm_ler, "trans6_ler": t5_ler,
            "ratio": ratio,
        }
        ...
        if len(mwpm_lers) >= 2:
            plot_results(
                ps[:len(mwpm_lers)], mwpm_lers, trans6_lers,
    ...
    for r in results:
        print(f"{r['p']:>8.4f}  {r['obs_rate']:>8.4f}  "
              f"{r['mwpm_ler']:>10.6f}  {r['trans6_ler']:>10.6f}  {r['ratio']:>8.2f}x")
    print(f"{'='*72}")
