#!/usr/bin/env python3
"""
run_train.py - Train single-basis AlphaQubit-like QEC Transformer

Trains a separate model for each basis (X or Z), following the AlphaQubit paper
approach of training independent decoders per basis.

Usage:
    python run_train.py --basis z --distance 3
    python run_train.py --basis x --distance 5 --num_steps 1000
    python run_train.py --bases z x --distance 3    # Train both sequentially
    python run_train.py --bases z x --distances 3 5 7  # Train all combos
...
# IMPORTANT: Import local (trans4) model BEFORE adding trans3 to sys.path.
# This ensures we get the single-classifier model, not trans3's dual-head version.
...
def main():
    parser = argparse.ArgumentParser(
        description="Train single-basis AlphaQubit-like QEC Transformer",
